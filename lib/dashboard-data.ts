...
export async function getSystemStatus(): Promise<SystemStatus> {
	...
	return {
		...
		activeProcesses:
			(readJsonFile(join(STATE_DIR, "active-processes.json")) as Record<string, unknown>[]) ?? [],
		schedule: (readJsonFile(join(STATE_DIR, "schedule.json")) as Record<string, unknown>[]) ?? [],
		scheduleState:
			(readJsonFile(join(STATE_DIR, "schedule-state.json")) as Record<string, unknown>) ?? {},
		proactive: {
			interventions: (
				((proactiveState as Record<string, unknown> | null)?.interventions as Record<
					string,
					unknown
				>[]) ?? []
			).filter(
				(i) =>
					Date.now() - new Date((i as { timestamp: string }).timestamp).getTime() <
					24 * 60 * 60 * 1000
			),
			lastCheck:
				((proactiveState as Record<string, unknown> | null)?.lastCheck as string | null) ?? null,
		},
...
export function getEventStats(events: Record<string, unknown>[]): EventStats {
	...
	const today = events.filter(
		(e) => now - new Date(e.ts as string).getTime() < 24 * 60 * 60 * 1000
	);
	return {
