...
export function formatContext(ctx: ScreenContext): string {
  ...
  const parts: string[] = [`Screen context (${ctx.timeRange.start.slice(11, 16)}–${ctx.timeRange.end.slice(11, 16)}):`];

  if (ctx.continuousActivityMinutes > 0) {
    parts.push(`**Continuous activity:** ${ctx.continuousActivityMinutes} min without a break (gap > 5 min)`);
  }
