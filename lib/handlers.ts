...
export async function handleVoice(chatId: number, messageId: number, fileId: string): Promise<void> {
  try {
    ...
    await dispatchToConversation(chatId, messageId,
...
export async function handlePhoto(chatId: number, messageId: number, fileId: string, caption: string): Promise<void> {
  try {
...
export async function handleText(chatId: number, messageId: number, text: string, isSmsBot: boolean): Promise<void> {
  if (isSmsBot) {
    await dispatchToConversation(chatId, messageId,
    ...
    await dispatchToConversation(chatId, messageId,
      `[Message from Randy via Telegram] ${text}`
    );
  }
}
