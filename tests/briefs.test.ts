...
import { buildBrief, BRIEF_TYPE_MAP, type BriefType } from "../lib/briefs/index";
import { buildFullBrief, buildMiddayBrief, buildEveningBrief, buildScheduledBrief } from "../lib/briefs/scheduled";
import { buildMessageBrief, buildLocationBrief } from "../lib/briefs/conversation";
...
describe("BRIEF_TYPE_MAP", () => {
  ...
  test("all values are valid BriefTypes", () => {
    const validTypes = new Set<BriefType>(["boot", "morning", "midday", "evening", "message", "location", "scheduled", "proactive"]);
...
describe("detectTriggers", () => {
  ...
  test("returns empty array when no triggers match", () => {
    const ctx: ScreenContext = {
      ...baseCtx,
      apps: [{ appName: "Xcode", windowTitles: ["MyApp.xcodeproj"], durationMinutes: 30, contentSample: [] }],
  ...
  test("handles multiple apps — only social ones trigger", () => {
    const ctx: ScreenContext = {
      ...
      apps: [
        ...
        { appName: "Safari", windowTitles: ["instagram.com"], durationMinutes: 25, contentSample: [] },
      ],
    };
    const triggers = detectTriggers(ctx);
...
async function buildBriefIsolated(type: BriefType | string, extra?: Record<string, string>): Promise<string> {
...
describe("buildBrief routing", () => {
  ...
  test("scheduled type passes prompt and taskName through", async () => {
    const result = await buildBriefIsolated("scheduled", { prompt: "check reminders", taskName: "reminder-check" });
