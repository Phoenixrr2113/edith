...
export class ReflectorSession {
	...
	private buildReflectorPrompt(
		...
		sections.push(
			`You are a reflection agent monitoring a running AI session. Your job is to analyze progress and provide a concise, contextual injection that re-grounds the main agent.`
		);

		sections.push(`## Original Request\n${this.originalPrompt.slice(0, 2000)}`);

		sections.push(
			`## Session Activity (${this.toolCallCount} tool calls so far)\n${compressedTranscript}`
		);
		...
		if (trigger === "compaction") {
			sections.push(
				`CRITICAL: The main agent's context was just compacted (compressed). Earlier conversation details are now summarized or lost. Your reflection MUST re-state the original goal, summarize key progress, and clearly list what remains to be done. This is the most important injection you can make.`
			);
		}

		if (trigger === "periodic") {
			sections.push(
				`The main agent has been working for ${this.toolCallCount} tool calls. Check for drift, wasted steps, or missed requirements. If the agent is on track, respond with exactly "no injection needed".`
			);
		}
