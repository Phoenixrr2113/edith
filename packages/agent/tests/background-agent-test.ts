...
for await (const message of handle) {
	...
	if (subtype === "task_started") {
		...
		console.log(
			`  ✅ TASK STARTED: id=${m.task_id}, desc="${m.description}", prompt="${(m.prompt ?? "").slice(0, 80)}"`
		);
	...
	if (subtype === "task_progress") {
		const m = message as any;
		console.log(
			`  📊 TASK PROGRESS: id=${m.task_id}, tool=${m.last_tool_name}, tokens=${m.usage?.total_tokens}, tools=${m.usage?.tool_uses}`
		);
	...
	if (subtype === "task_notification") {
		const m = message as any;
		console.log(
			`  🏁 TASK COMPLETE: id=${m.task_id}, status=${m.status}, summary="${(m.summary ?? "").slice(0, 100)}"`
		);
	...
	if (type === "result") {
		const m = message as any;
		console.log(
			`  🎯 RESULT: cost=$${m.total_cost_usd?.toFixed(4)}, turns=${m.num_turns}, error=${m.is_error}`
		);
	}
}
