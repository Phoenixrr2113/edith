...
### Brief Building (`lib/briefs.ts`, `lib/briefs/scheduled.ts`)
Before Claude wakes, Edith pre-fetches calendar and email from n8n and assembles a context brief. 8 brief types: boot, morning, midday, evening, message, location, scheduled, proactive. Brief builders in lib/briefs/scheduled.ts assemble context for each scheduled task.
...
Files: `packages/agent/.state/activity/YYYY-MM-DD.md`, never rotated. MCP tool `get_activity`: `days=1` (today), `days=7` (week), `days=30` (month). L2 summaries returned when available, L1 as fallback.
...
| Brief building | lib/briefs.ts, lib/briefs/scheduled.ts | 8 brief types, pre-wake calendar/email fetch |
...
| Event logging | lib/state.ts | packages/agent/.state/events.jsonl — every dispatch, cost, error, schedule fire, timestamped |
...
| Time-based reminders | checkTimeReminders() every 5 min | SQLite reminders table, checked by reminder-checker agent |
| Named locations | SQLite locations table + location-latest.json | GPS coords for geofence definitions |
...
|---------|----------------------|-----------------|
| Time-based reminders | `checkTimeReminders()` every 5 min, SQLite reminders table | Same logic, SQLite-backed |
| Scheduled tasks | scheduler.ts + SQLite schedule table | Same logic, SQLite-backed |
...
| Activity logging | activity.ts → packages/agent/.state/activity/ | Same logic, cloud storage |
...
| Taskboard | taskboard.ts → packages/agent/.state/taskboard.md | Same logic, SQLite-backed |
| Brief building | briefs.ts + briefs/scheduled.ts | Same logic, calendar/email fetched via MCP tools |
...
| Fix file race conditions | lib/state.ts, mcp/server.ts | Add file locking (flock or write-to-temp+rename) for taskboard.md, SQLite reminders table, events.jsonl. No more corrupted reads. |
...
|------|-------|-------|-------------------|
| Taskboard archival before rotation | lib/taskboard.ts | #7 | `rotateTaskboard()` writes pruned entries to `packages/agent/.state/taskboard-archive/YYYY-MM.md` (append-only) before deleting. New export: `getTaskboardArchive(months)` for review agents. Existing rotation behavior unchanged. |
...
| lib/briefs/scheduled.ts | Preserved as-is during briefs.ts split. Assembles context briefs. Calendar/email is fetched by Claude via MCP tools during execution. |
...
| Migrate state from packages/agent/.state/ JSON files to SQLite | All JSON files (SQLite reminders table, SQLite schedule table, proactive-state.json, SQLite locations table, dead-letters.json) → SQLite tables. events.jsonl → SQLite events table. taskboard.md → SQLite taskboard table. Migration script that converts existing data. |
