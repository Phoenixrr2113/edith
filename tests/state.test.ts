...
import { writeFileSync, readFileSync, appendFileSync, existsSync, unlinkSync, statSync } from "fs";
...
describe("event logging", () => {
  ...
  test("logEvent appends JSONL line with timestamp and type", () => {
    ...
    const logEvent = (type: string, data: Record<string, any> = {}) => {
      appendFileSync(eventsFile, JSON.stringify({ ts: new Date().toISOString(), type, ...data }) + "\n", "utf-8");
    };
  ...
  test("rotateEvents prunes old lines when file > 1MB", () => {
    ...
    for (let i = 0; i < 8000; i++) {
      appendFileSync(eventsFile, JSON.stringify({ ts: oldTs, type: "old", padding: "x".repeat(120) }) + "\n", "utf-8");
    }
    for (let i = 0; i < 100; i++) {
...
describe("dead letter queue", () => {
  ...
  test("save / load / clear lifecycle", () => {
    ...
    // Load
    const loaded = readFileSync(dlFile, "utf-8").split("\n").filter(Boolean).map(l => JSON.parse(l));
  ...
  test("multiple dead letters accumulate", () => {
    for (let i = 0; i < 3; i++) {
      appendFileSync(dlFile, JSON.stringify({ ts: new Date().toISOString(), chatId: i, message: `msg${i}`, error: "err" }) + "\n", "utf-8");
    }
    const loaded = readFileSync(dlFile, "utf-8").split("\n").filter(Boolean).map(l => JSON.parse(l));
...
describe("active processes", () => {
  test("write and read active processes", () => {
    ...
    const processes = [
      { pid: 1, label: "morning-brief", startedAt: new Date().toISOString(), prompt: "do morning stuff" },
