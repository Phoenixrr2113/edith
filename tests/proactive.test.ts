...
function loadState(): ProactiveState {
	...
	try {
		...
		return { interventions: [], lastCheck: "" };
	}
}
...
function canIntervene(
	...
	if (existsSync(configFile)) {
		try {
			toggle = JSON.parse(readFileSync(configFile, "utf-8"));
		} catch {}
	}
	...
	if (category) {
		...
		const lastSame = state.interventions
...
describe("recordIntervention", () => {
	test("persists intervention and trims to 24h", () => {
		...
		saveState({
			interventions: [{ timestamp: old, category: "old", message: "stale" }],
			lastCheck: "",
		});
