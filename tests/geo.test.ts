...
interface LocationEntry { name: string; label: string; lat: number; lon: number; radiusMeters: number; }
interface Reminder { id: string; text: string; type: "time" | "location"; location?: string; radiusMeters?: number; fireAt?: string; fired: boolean; created: string; }
...
function loadLocations(file: string): LocationEntry[] {
  try { return JSON.parse(readFileSync(file, "utf-8")).locations ?? []; } catch { return []; }
}
...
describe("checkLocationReminders (reimplemented)", () => {
  ...
  test("triggers reminder when within radius", () => {
    saveLocations(locFile, [{ name: "home", label: "Home", lat: 40.7128, lon: -74.006, radiusMeters: 500 }]);
    saveReminders(remFile, [
      { id: "r1", text: "Take out trash", type: "location", location: "home", fired: false, created: new Date().toISOString() },
    ]);
  ...
  test("does not trigger when outside radius", () => {
    saveLocations(locFile, [{ name: "home", label: "Home", lat: 40.7128, lon: -74.006, radiusMeters: 100 }]);
    saveReminders(remFile, [
      { id: "r1", text: "Take out trash", type: "location", location: "home", fired: false, created: new Date().toISOString() },
    ]);
  ...
  test("skips fired reminders", () => {
    saveLocations(locFile, [{ name: "home", label: "Home", lat: 40.7128, lon: -74.006, radiusMeters: 500 }]);
    saveReminders(remFile, [
      { id: "r1", text: "Already done", type: "location", location: "home", fired: true, created: new Date().toISOString() },
    ]);
  ...
  test("skips time-based reminders", () => {
    saveLocations(locFile, [{ name: "home", label: "Home", lat: 40.7128, lon: -74.006, radiusMeters: 500 }]);
    saveReminders(remFile, [
      { id: "r1", text: "Time reminder", type: "time", fireAt: new Date().toISOString(), fired: false, created: new Date().toISOString() },
    ]);
...
describe("checkTimeReminders (reimplemented)", () => {
  ...
  function checkTimeReminders() {
    const reminders = loadReminders(remFile);
    const now = Date.now();
    return reminders.filter(r => !r.fired && r.type === "time" && r.fireAt && new Date(r.fireAt).getTime() <= now);
  }

  test("returns reminders with past fireAt", () => {
    const past = new Date(Date.now() - 60_000).toISOString();
    saveReminders(remFile, [
      { id: "r1", text: "Due now", type: "time", fireAt: past, fired: false, created: new Date().toISOString() },
    ]);
  ...
  test("ignores future reminders", () => {
    ...
    saveReminders(remFile, [
      { id: "r1", text: "Not yet", type: "time", fireAt: future, fired: false, created: new Date().toISOString() },
    ]);
  ...
  test("ignores fired reminders", () => {
    const past = new Date(Date.now() - 60_000).toISOString();
    saveReminders(remFile, [
      { id: "r1", text: "Already fired", type: "time", fireAt: past, fired: true, created: new Date().toISOString() },
    ]);
    expect(checkTimeReminders()).toHaveLength(0);
  });
});
...
describe("checkLocationTransitions (reimplemented)", () => {
  ...
  interface LocationTransition { type: "arrived" | "departed"; locationName: string; locationLabel: string; }
  ...
  function checkLocationTransitions(lat: number, lon: number): LocationTransition[] {
    ...
    if (currentLocationName) {
      ...
      transitions.push({ type: "departed", locationName: currentLocationName, locationLabel: prev?.label ?? currentLocationName });
    }
    if (newName && atLocation) {
      transitions.push({ type: "arrived", locationName: newName, locationLabel: atLocation.label ?? newName });
    }
    currentLocationName = newName;
    return transitions;
  }
  ...
  test("arriving at a location triggers arrived event", () => {
    saveLocations(locFile, [{ name: "office", label: "Office", lat: 40.75, lon: -73.99, radiusMeters: 200 }]);

    // Initialize somewhere else
    checkLocationTransitions(40.70, -74.01);
  ...
  test("depart + arrive when moving between locations", () => {
    saveLocations(locFile, [
      { name: "home", label: "Home", lat: 40.71, lon: -74.00, radiusMeters: 300 },
      { name: "office", label: "Office", lat: 40.75, lon: -73.99, radiusMeters: 300 },
    ]);

    // Initialize at home
    checkLocationTransitions(40.71, -74.00);
  ...
  test("no transition when staying at same location", () => {
    saveLocations(locFile, [{ name: "home", label: "Home", lat: 40.71, lon: -74.00, radiusMeters: 300 }]);
    checkLocationTransitions(40.71, -74.00);
