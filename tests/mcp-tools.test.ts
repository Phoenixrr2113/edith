...
describe("MCP: save_location → list_locations round-trip", () => {
  ...
  function saveLocation(name: string, label: string, lat: number, lon: number, radiusMeters: number = 200) {
  ...
  test("save and retrieve location", () => {
    ...
    expect(locs[0]).toEqual({ name: "home", label: "Home", lat: 40.7128, lon: -74.006, radiusMeters: 300 });
  });

  test("upsert overwrites existing location", () => {
    saveLocation("home", "Home", 40.71, -74.00, 200);
  ...
  test("multiple locations coexist", () => {
    saveLocation("home", "Home", 40.71, -74.00);
...
describe("MCP: save_reminder → list → mark_fired lifecycle", () => {
  ...
  test("time reminder: create → list → fire", () => {
    const r = saveReminder({ text: "Call dentist", type: "time", fireAt: new Date(Date.now() + 3600_000).toISOString() });
  ...
  test("location reminder: create and check via geofence", () => {
    ...
    saveJson(locFile, { locations: [{ name: "grocery", label: "Grocery Store", lat: 40.73, lon: -73.99, radiusMeters: 200 }] });

    const r = saveReminder({ text: "Buy eggs", type: "location", location: "grocery" });

    // Check: within radius
    const locations = loadJson<any>(locFile, { locations: [] }).locations;
    const reminders = listReminders().filter((r: any) => !r.fired && r.type === "location" && r.location);
    const triggered = [];
...
describe("MCP: proactive_history + record_intervention round-trip", () => {
  ...
  function recordIntervention(category: string, message: string, tsOverride?: string) {
    ...
    state.interventions.push({ timestamp: tsOverride ?? new Date().toISOString(), category, message: message.slice(0, 200) });
    const cutoff = Date.now() - 24 * 60 * 60 * 1000;
    state.interventions = state.interventions.filter((i: any) => new Date(i.timestamp).getTime() > cutoff);
...
function getNotificationRoute(channel: string): { method: string; prefix?: string; endpoint?: string } {
  switch (channel) {
    case "telegram": return { method: "telegram" };
    case "whatsapp": return { method: "twilio", prefix: "whatsapp:" };
    case "sms": return { method: "twilio" };
    case "email": return { method: "n8n", endpoint: "notify" };
    case "slack": return { method: "n8n", endpoint: "notify" };
    case "discord": return { method: "n8n", endpoint: "notify" };
    case "desktop": return { method: "desktop" };
    case "dialog": return { method: "dialog" };
