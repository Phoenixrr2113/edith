 ...
 */
export async function extractFromAudio(transcripts: AudioTranscript[]): Promise<ExtractedKnowledge | null> {
  if (!OPENROUTER_API_KEY) return null;
  if (transcripts.length === 0) return null;

  // Combine transcripts into a single block
  const combined = transcripts
    .map((t) => `[${t.timestamp.slice(11, 19)}] ${t.text}`)
    .join("\n");
 ...
 */
export async function processAudioTranscripts(transcripts: AudioTranscript[]): Promise<ExtractedKnowledge | null> {
