	...
	test("formats apps and audio correctly", () => {
		const ctx: ScreenContext = {
			...
			apps: [
				{
					appName: "VS Code",
					windowTitles: ["edith.ts", "dispatch.ts"],
					durationMinutes: 12,
					contentSample: ["function dispatch() { ... }"],
				},
	...
	test("limits apps to 5 and audio to 5", () => {
		const ctx: ScreenContext = {
			...
			audioTranscripts: Array.from({ length: 10 }, (_, i) => ({
				timestamp: `2026-03-26T10:0${i}:00Z`,
				text: `Transcript ${i}`,
			})),
			continuousActivityMinutes: 0,
			empty: false,
	...
	test("limits content samples to 5 per app", () => {
		const items: SearchItem[] = Array.from({ length: 10 }, (_, i) => ({
			...
			content: {
				app_name: "App",
				text: "x".repeat(30),
				timestamp: `2026-03-26T10:${String(i).padStart(2, "0")}:00Z`,
			},
		}));
...
describe("extractAudio", () => {
	test("extracts transcriptions from Audio items", () => {
		const items: SearchItem[] = [
			{
				type: "Audio",
				content: { transcription: "Hello world", timestamp: "2026-03-26T10:00:00Z" },
			},
...
describe("measureContinuousActivity", () => {
	test("returns 0 for < 2 items", () => {
		...
		expect(
			measureContinuousActivity([{ type: "OCR", content: { timestamp: "2026-03-26T10:00:00Z" } }])
		).toBe(0);
	});
